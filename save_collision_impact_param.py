...
def save_collision(relaxed_data, output, separation_r, impact_speed, collision_axis=0):
    ...
    # calculating radius of planet 
    radius = np.max(np.linalg.norm(positions, axis=1))
    
    # scaling factor: 0.0 (head-on), 0.5 (oblique), 0.7 (grazing), 1 (miss)
    b_factor = 0.5 
    impact_parameter = b_factor * (radius * 2)
    positions_2[:, 1] += impact_parameter
...
if __name__ == "__main__":
    save_collision(relaxed_data='./data/Planet300_relaxed.dat', output='./data/Planet300_collision_impact_param.dat', separation_r=5e9, impact_speed=1300)